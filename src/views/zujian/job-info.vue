<template>
  <div class="job-info">
    ...
    <el-form label-width="220px">
      ...
      <!-- 从业信息 -->
      <el-form-item>
        <el-button type="primary" @click="saveEmployeeJobInfo">保存更新</el-button>
...
<script>
import EmployeeEnum from '@/views/zujian/job-info.vue'
import { getEmployeeJobInfo, saveEmployeeJobInfo } from '@/api/employees'
export default {
  data() {
    return {
  ...
  created() {
    this.getEmployeeJobInfo()
    this.saveEmployeeJobInfo()
  },
  methods: {
    async getEmployeeJobInfo() {
      this.formData = await getEmployeeJobInfo(this.userId)
    },
    async saveEmployeeJobInfo() {
      await saveEmployeeJobInfo(this.formData)
      this.$messafe.success('岗位成功')
    }
  }
}
</script>
